    ...
    <header className="h-16 bg-canvas/95 backdrop-blur-md border-b border-line-muted p-2 flex items-center justify-between gap-6 shrink-0 sticky top-0 z-10">
      ...
      <div className="relative flex-1 max-w-sm h-full mx-auto group">
        ...
        <input
          ...
          className="w-full h-full pl-10 pr-10 text-sm bg-input backdrop-blur-sm border border-line/60 rounded-full focus:outline-none transition-all duration-200 text-copy placeholder-muted/70"
        />
        {searchQuery && (
          <button
            onClick={() => setSearchQuery("")}
            className="p-1 hover:bg-line/30 rounded-full absolute right-2.5 top-1/2 -translate-y-1/2 text-dim hover:text-copy cursor-pointer transition-all duration-150"
          >
